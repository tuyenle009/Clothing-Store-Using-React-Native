...
module.exports = {
  ...
  clearByUserId: (req, res) => {
  const { user_id } = req.query;
  if (!user_id) {
    return res.status(400).json({ message: 'User ID is required' });
  }

  Cart.clearByUserId(user_id, (err, result) => {
    if (err) {
      console.error('Error clearing user cart:', err);
      return res.status(500).json({ message: 'Failed to clear user cart' });
    }
    res.json({ message: 'User cart cleared successfully', result });
  });
},
};
